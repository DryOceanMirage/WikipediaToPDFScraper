...
if curl -s "$NEXTPAGE" | grep -q "Next page"; then
    ...
    curl "$NEXTPAGE" | grep -A 360 '<ul class="mw-allpages-chunk">' | sed -n 's/.*>\(.*\)<\/a>.*/\1/p' | grep -v 'Next page'| grep -v 'http' | grep -v 'Privacy policy' | grep -v 'About Wikipedia' | grep -v 'Disclaimers' | sed 's/&quot;/\"/g' | sed "s/&#039;/'/g" >> titles.txt && PREVPAGE="$NEXTPAGE" && getNextPage
